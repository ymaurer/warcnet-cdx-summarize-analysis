...
```{r}
setwd("/Users/au148767/Projects/cdx-summarize/")
```
...
df_distinct_domains_overall <- read_data_files("data", "*distinct-domains-overall.csv", c("n")) %>% 
...
# TO DO: find out how to change labels

# can also be used with filter: filter(archive=="bnf") for only bnf OR filter(archive!="bnf") for all archives except bnf (!= means not equal to)
# to filter on more than once thing, use fx filter(archive %in% c("cc", "ia")), see example:

# clean_domains_per_year  %>%
#  filter(archive %in% c("cc", "ia")) %>% 
#  ggplot(aes(x=year, y=count/1000000)) +
#  geom_bar(stat="identity") +
#  facet_wrap(~collection) +
#  theme_minimal() +
