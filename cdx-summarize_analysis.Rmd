...
line_colors_lu_subset <- c("#838383","#c9c9c9","#e5e5e5")
line_labels_lu_subset <- c("Bibliothèque nationale de France", "Netarkivet", "Hungary Web Archive")
line_types_lu_subset <- c("twodash","dotted","longdash")

lu_across_archives_melt %>% 
  filter(archive == "szl" | archive == "bnf" | archive == "nak") %>%
...
fr_across_archives <- fr_across_archives %>% mutate_at(c("bnf","cc","ia","lwa","nak","szl"), as.numeric) # remember to add "nak" when adding the DK data
...
# sort colors and labels in the right order
line_colors_fr_subset <- c("#aeaeae","#c9c9c9","#e5e5e5")
line_labels_fr_subset <- c("Luxembourg Web Archive", "Netarkivet", "Hungary Web Archive")
line_types_fr_subset <- c("solid","dotted","longdash")

fr_across_archives_melt %>% 
  filter(archive == "lwa" | archive == "nak" | archive == "szl") %>%
#  filter(!year %in% c("1996","1997","1998","1999","2000","2001","2002","2003","2004")) %>%
  filter(n>0) %>% # remove the years where there are no data
  ggplot(aes(x = year, y = n, group = archive)) +
  geom_point(color="darkgrey") + # added to be able to see BNF 
