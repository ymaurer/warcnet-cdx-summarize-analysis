...
# 4. Top 20 TLDs in the different national archives
...
percentage_top20_dk <- dk_lvl2_per_tld %>% 
...
percentage_top20_dk %>% 
  ...
  labs(y = "Percentage from top 20 TLDs in Netarkivet \n", x = "\n TLD")  
...
percentage_top20 %>% ggplot(aes(x=reorder(TLD, -percentage) %>% fct_relevel("Other", after = Inf), y=percentage)) + 
  ...
  theme_minimal() +
  scale_y_continuous(labels = percent) +
  labs(y = "Percentage from top 20 TLDs in LWA \n", x = "\n TLD")  
...
percentage_top20 %>% ggplot(aes(x=reorder(TLD, -percentage) %>% fct_relevel("Other", after = Inf), y=percentage)) + 
  ...
  theme_minimal() +
  scale_y_continuous(labels = percent) +
  labs(y = "Percentage from top 20 TLDs in BNF \n", x = "\n TLD")  
...
percentage_top20 %>% ggplot(aes(x=reorder(TLD, -percentage) %>% fct_relevel("Other", after = Inf), y=percentage)) + 
  ...
  theme_minimal() +
  scale_y_continuous(labels = percent) +
  labs(y = "Percentage from top 20 TLDs in SZL \n", x = "\n TLD")  
...
# 6. Distinct TLD per year in the national archives (not BNF)
Based on distinct-tld-per-year.csv (the ones with all)
```{r}
...
# write to csv (optional)
# write_csv(distinct_tlds_per_year, "results/distinct_tlds_per_year.csv")


szl_tld_per_year <- read.csv("data/szl-all-statb-distinct-tld-per-year.csv", header = FALSE) 
colnames(szl_tld_per_year) <- c("year","szl") # name columns

lwa_tld_per_year <- read.csv("data/lwa-all-statb-distinct-tld-per-year.csv", header = FALSE) 
colnames(lwa_tld_per_year) <- c("year","lwa") # name columns

nak_tld_per_year <- read.csv("data/nak-all-statb-distinct-tld-per-year.csv", header = FALSE) 
colnames(nak_tld_per_year) <- c("year","nak") # name columns

distinct_tlds_per_year_df_prelim <- merge(szl_tld_per_year,lwa_tld_per_year,by = "year", all=TRUE)
distinct_tlds_per_year_df <- merge(distinct_tlds_per_year_df_prelim,nak_tld_per_year,by = "year", all=TRUE)
