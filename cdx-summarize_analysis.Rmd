...
# Load libraries
```{r}
# do we need an install.packages?? 
...
## 3 .lu
TO DO: add "nak" when adding the Danish data and Netarkivet i labels
```{r}
...
lu_across_archives <- lu_across_archives[-c(1:4,31:32),] # remove row 1-4 and 31-32 (archive names + years with no data)  
...
lu_across_archives_melt %>% 
  mutate(archive = fct_relevel(archive, "lwa", "ia", "cc", "bnf", "szl")) %>%
  ggplot(aes(x = year, y = n, group = archive, color = archive)) + 
  geom_point() + 
  ...
  theme(axis.text=element_text(size=12, face = "bold"),axis.title=element_text(size=12, face = "bold")) +
  scale_y_continuous(labels = function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = F)) +
  scale_colour_manual(values = c("deeppink2","navyblue","orange2","gold","darkorchid3"), labels = c("Luxembourg Web Archive","Internet Archive","Common Crawl","Bibliothèque nationale de France","Hungary Web Archive")) +
  labs(y = "Number of domains \n", x = "\n Year", color = "Archive") 
# to export: 1555 x 530 
...
lu_across_archives_melt %>% 
  ...
  theme(axis.text=element_text(size=12, face = "bold"),axis.title=element_text(size=12, face = "bold")) +
  scale_y_continuous(labels = function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = F)) +
  scale_colour_manual(values = c("gold", "darkorchid3"), labels = c("Bibliothèque nationale de France","Hungary Web Archive")) + 
  labs(y = "Number of domains \n", x = "\n Year", color = "Archive")
```
...
fr_across_archives <- fr_across_archives[-c(1:8,31:32),] # remove row 1-4 and 31-32 (archive names + years with no data)  
...
fr_across_archives <- fr_across_archives %>% mutate_at(c("bnf", "cc", "ia", "lwa", "szl"), as.numeric) # remember to add "nak" when adding the DK data
...
fr_across_archives_melt %>% 
  mutate(archive = fct_relevel(archive,"bnf","ia", "cc","lwa","szl")) %>%
  ggplot(aes(x = year, y = n, group = archive, color = archive)) + 
  geom_point() + 
  geom_line(linewidth = 1.2) + 
  theme_minimal() +
  ...
  theme(axis.text=element_text(size=12, face = "bold"),axis.title=element_text(size=12, face = "bold")) +
  scale_y_continuous(labels = function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = F)) +
  scale_colour_manual(values = c("gold","navyblue","orange2","deeppink2","darkorchid3"), labels = c("Bibliothèque nationale de France","Internet Archive","Common Crawl","Luxembourg Web Archive","Hungary Web Archive")) +
  labs(y = "Number of domains \n", x = "\n Year", color = "Archive") 

# The numbers for Luxembourg and Hungary are so small that we can't really see them. If we want to see them, we could filter
# first data in 2014
fr_across_archives_melt %>% 
  filter(archive == "lwa" | archive == "szl") %>%
...
hu_across_archives <- hu_across_archives[-c(1:4,31:32),] # remove row 1-4 and 31-32 (archive names + years with no data)  
...
hu_across_archives <- hu_across_archives %>% mutate_at(c("bnf", "cc", "ia", "lwa", "szl"), as.numeric) # remember to add "nak" when adding the DK data
...
hu_across_archives_melt %>% 
  mutate(archive = fct_relevel(archive,"szl","ia", "cc","bnf","lwa")) %>%
  ggplot(aes(x = year, y = n, group = archive, color = archive)) + 
  geom_point() + 
  geom_line(linewidth = 1.2) + 
  theme_minimal() +
  ...
  theme(axis.text=element_text(size=12, face = "bold"),axis.title=element_text(size=12, face = "bold")) +
  scale_y_continuous(labels = function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = F)) +
  scale_colour_manual(values = c("darkorchid3","navyblue","orange2","gold","deeppink2"), labels = c("Hungary Web Archive","Internet Archive","Common Crawl","Bibliothèque nationale de France","Luxembourg Web Archive")) +
  labs(y = "Number of domains \n", x = "\n Year", color = "Archive") 

# The numbers for BNF and Luxembourg are so small that we can't really see them. If we want to see them, we could filter
# first data in 2016
hu_across_archives_melt %>% 
  filter(archive == "bnf" | archive == "lwa") %>%
  ...
  theme(axis.text=element_text(size=12, face = "bold"),axis.title=element_text(size=12, face = "bold")) +
  scale_y_continuous(labels = function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = F)) +
  scale_colour_manual(values = c("gold", "deeppink2"), labels = c("Bibliothèque nationale de France","Luxembourg Web Archive")) + 
  labs(y = "Number of domains \n", x = "\n Year", color = "Archive")
```
...
#### TO DO: CHANGE ORDER IN PLOT using factor relevel, change order accordingly
dk_across_archives_melt %>% 
  mutate(archive = fct_relevel(archive, "lwa", "ia", "cc", "bnf", "szl")) %>% 
  ggplot(aes(x = year, y = n, group = archive, color = archive)) + 

#### TO DO: CHANGE COLOURS + LABELS  
# plot
...
# The numbers for BNF, Luxembourg, Hungary and BNF are so small that we can't really see them. If we want to see them, we could filter
### remember to check that colours and labels match
dk_across_archives_melt %>% 
  ...
  theme(axis.text=element_text(size=14, face = "bold"),axis.title=element_text(size=16, face = "bold")) +
  scale_y_continuous(labels = function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = F)) +
 # scale_colour_manual(values = c("gold", "deeppink2","darkorchid3"), labels = c("BNF","Luxembourg Web Archive", "Hungary Web Archive")) + 
  labs(y = "Number of domains \n", x = "\n Year", color = "Archive")
...
```{r}
# read data
overlaps_hosts_szl <- read_csv2("data_overlaps/overlaps_hosts_szl.csv")
...
overlaps_hosts_szl_melt %>% ggplot(aes(x=Year, y=n, fill=Archive)) +
  ...
  scale_y_continuous(labels = function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = F)) +
  scale_x_continuous(breaks = clean_domains_per_year$year) +
  scale_fill_manual(values = c("orange2","navyblue","darkorchid3","aquamarine","cadetblue2","cornflowerblue","cyan"),
  labels = c("Common Crawl","Internet Archive","SZL","Internet Archive & Common Crawl","SZL & Common Crawl","SZL & Internet Archive","SZL & Internet Archive & Common Crawl")) + 
  labs(y = "Number of .dk domains \n", x = "\n Year", color = "Archive")
```
